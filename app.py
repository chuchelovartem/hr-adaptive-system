...
def get_adaptive_question_prompt(role, pos, step, max_steps):
    if role == "Соискатель":
        # МАТРИЦА РАЗНОСТОРОННЕЙ ОЦЕНКИ
        domains = [
            "Фундаментальные инженерные знания и профильная терминология",
            "Практический кейс: решение типовой рабочей задачи на объекте",
            "Специализированное программное обеспечение, САПР и цифровые инструменты",
            "Нормативная документация, ГОСТы, СНиПы и техника безопасности",
            "Анализ рисков и поиск ошибок в смежных разделах проекта",
            "Оптимизация процессов, экономика проекта или работа с заказчиком"
        ]
        ...
        🚨 ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ: Тема твоего вопроса должна строго относиться к области: "{current_domain}". 
        КАРДИНАЛЬНО СМЕНИ ТЕМУ! Даже если прошлый ответ кандидата был глупым или неполным, не пытайся его уточнять. Переходи строго к новой указанной области.
