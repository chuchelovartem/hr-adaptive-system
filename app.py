...
def get_final_analysis_prompt(role, pos, jd_context, transcript):
    ...
    anti_break_rule = "\n[КРИТИЧЕСКАЯ ИНСТРУКЦИЯ] КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать фигурные скобки { и } в тексте отчета. Используй их ТОЛЬКО один раз в самом конце для генерации JSON."
    
    if role == "Соискатель":
        return f"""Ты — объективный HR-директор и сертифицированный аудитор. Проведи аудит интервью на позицию {pos}.{context_block}{anti_break_rule}
        
        [МЕТОДОЛОГИЯ ОЦЕНКИ BARS]
        1-3: Отвечает общими фразами, уходит от сути (Риск).
        4-6: Знает теорию, но плавает в практике (Junior/Middle-).
        7-8: Приводит конкретные примеры, решает кейс (Уверенный спец).
        9-10: Видит систему целиком, учитывает риски (Senior/Lead).
        ...
        return f"""Ты — корпоративный HR-методолог по Talent Management. Проведи аудит сотрудника: {pos}.{context_block}{anti_break_rule}
        
        [МЕТОДОЛОГИЯ ОЦЕНКИ BARS]
        1-3: Выгорание, сопротивление изменениям.
        4-6: Исполнитель. Выполняет задачи, безынициативен.
        7-8: Самостоятелен, быстро обучается (Готов к повышению).
        9-10: Бизнес-ориентирован, ментор (HiPo).
        
        [СТРУКТУРА ОТВЕТА]
        ШАГ 1: ТЕКСТОВЫЙ ОТЧЕТ
        - 9-Box Grid Analysis (Performance vs Potential).
        - Skill-Gap Analysis (Дефициты компетенций).
        - ИПР (Индивидуальный план развития - 3 шага).
        - Вердикт.
