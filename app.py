...
def get_final_analysis_prompt(role, pos):
    """Многомерный анализ с NLP (тональность) и выдачей JSON для радара."""
    if role == "Соискатель":
        return f"""Ты — Senior HR-Аналитик. Проведи аудит диалога с кандидатом на позицию: {pos}.

        [ИННОВАЦИОННЫЙ АНАЛИЗ]
        1. Антифрод: Ищи саботаж, абсурд или следы переключения вкладок (если кандидат не уложился в тайминг).
        2. NLP Анализ Тональности: Оцени уверенность (Confidence). Использует ли он пассивный залог, оправдывается ли?
        3. Hard Skills: Оцени глубину технических знаний по ответам.

        СТРУКТУРА ТЕКСТОВОГО ОТЧЕТА:
        ### Антифрод-Радар: [Пройден / Провален]
        ### Лексический профиль и Уверенность: (Анализ тональности)
        ### Фактические Компетенции: (Разбор Hard Skills с цитатой)
        ### ИТОГОВЫЙ ВЕРДИКТ: [РЕКОМЕНДОВАН / ОТКЛОНЕН]

        [ВАЖНО: JSON ДЛЯ ДАШБОРДА]
        В самом конце отчета ОБЯЗАТЕЛЬНО выведи JSON-блок в формате:
        ```json
        {{
            "Hard_Skills": 8,
            "Когнитивная_Гибкость": 7,
            "Уверенность_и_Лидерство": 5,
            "Профессиональная_Этика": 9,
            "Системное_Мышление": 6
        }}
        ```
        Оценки от 0 до 10. Если выявлен саботаж — везде 0."""
    else:
        return f"""Ты — Эксперт по развитию. Проанализируй диалог сотрудника ({pos}).
...
def main():
    ...
    if st.session_state.step == "role_selection":
        ...
        if col1.button("Соискатель (Скрининг)", use_container_width=True):
            st.session_state.role = "Соискатель"
            st.session_state.max_questions = 8
            st.session_state.step = "pos_input"
            st.rerun()
        if col2.button("Сотрудник (Аудит)", use_container_width=True):
