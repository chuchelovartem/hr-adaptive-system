...
def get_adaptive_question_prompt(role, pos, jd_context, step, max_steps):
    ...
    if role == "Соискатель":
        ...
        [РАЗРЫВ КОНТЕКСТА] Забудь прошлую ситуацию. Придумай АБСОЛЮТНО НОВУЮ независимую рабочую задачу.
        
        [АБСОЛЮТНЫЙ ЗАПРЕТ НА ОЦЕНКУ И МЕТА-ТЕКСТ]
        НИКАК не комментируй прошлый ответ кандидата! 
        ЗАПРЕЩЕНО писать: "Неправильно", "Не подходит", "Продолжаем", "Новая ситуация", "Верно", "Давай уточним".
        ПРОСТО ВЫВЕДИ ТЕКСТ ВОПРОСА. 
        
        ПРАВИЛА: 
        1. Формат: "Представь ситуацию...". 
        2. АДАПТИРУЙ ЯЗЫК: Если профессия линейная или рабочая (дворник, грузчик), используй простой бытовой язык, без слов "алгоритм", "стратегия", "минимизация". Спрашивай жизненно и просто."""
    else:
...
def get_final_analysis_prompt(role, pos, jd_context, transcript, cheat_count):
    ...
    if role == "Соискатель":
        ...
        [КАЛИБРОВКА] Для линейных профессий короткие ответы (1-3 слова) — это норма, а не саботаж! Оценивай логику.
        [ЗАПРЕТ НА ВЫДУМЫВАНИЕ] При реальном хамстве или прямых отказах — пиши "Не выявлено" в сильных сторонах и ставь нули в JSON.
        
        ФОРМАТ: Резюме компетенций, Сильные стороны, Риски, Вердикт.
        
        САМОЕ ВАЖНОЕ: В самом конце ответа обязательно выведи блок кода JSON. Оценивай целыми числами от 0 до 10.
        ```json
        ...
        САМОЕ ВАЖНОЕ: В самом конце ответа обязательно выведи блок кода JSON. Оценивай целыми числами 0-10.
        ```json
...
def main():
    ...
    if st.session_state.step == "role_selection":
        ...
        with st.spinner("Формирование финального HR-аудита..."):
            ...
            # Улучшенный парсер JSON, который ищет скобки и игнорирует текст вокруг
            match = re.search(r'\{[^{}]*\}', raw, re.DOTALL)
            json_str = match.group(0) if match else "{}"

            try:
                radar_data = json.loads(json_str)
                text_report = raw.replace("```json", "").replace("```", "").replace(json_str, "").strip()
            except:
