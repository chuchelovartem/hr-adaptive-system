...
def get_adaptive_question_prompt(role, pos, jd_context, step, max_steps):
    ...
    grade_adaptation = f"""
        [АДАПТАЦИЯ СЛОЖНОСТИ ПОД ДОЛЖНОСТЬ: {pos} - КРИТИЧЕСКИ ВАЖНО]
        Оцени уровень должности. 
        - Если это РАБОЧИЙ/ЛИНЕЙНЫЙ ПЕРСОНАЛ (например: дворник, грузчик, кассир, слесарь): Ситуации должны быть максимально приземленными, бытовыми и из реальной физической практики (сломался инвентарь, напарник не вышел, недовольный клиент, снегопад). КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать корпоративную лексику ("дорожные карты", "KPI", "бюджетирование", "координация подрядчиков"). Проверка "роста" — это готовность стать старшим смены или бригадиром.
        - Если это ОФИСНЫЙ/УПРАВЛЕНЧЕСКИЙ ПЕРСОНАЛ: Используй бизнес-лексику, стратегические кейсы, метрики и управление проектами.
    """
    ...
    if role == "Соискатель":
        ...
        ШАГ: {step}/{max_steps}. Твоя НОВАЯ область проверки: "{current_domain}".{grade_adaptation}
        
        [ФОРМАТ ВОПРОСА]
        Твой ответ увидит кандидат. ЗАПРЕЩЕНО писать внутренние комментарии (например: "Проверяем знания...").
        ...
        **Вопрос:** (1 четкий, открытый вопрос, подразумевающий короткий ответ)
        ...
        [ЗАПРЕТЫ] СТРОГО ОДИН ВОПРОС. ЗАПРЕЩЕНО оценивать прошлый ответ ("Верно", "Не подходит")."""
    else:
        scenario = [
            "Ретроспектива (Оценка недавней сложной задачи в рамках текущих обязанностей)", 
            "Оценка потенциала (Моделирование ситуации на шаг сложнее привычной)", 
            "Аудит дефицитов (Оценка нехватки навыков или знаний)", 
            "Адаптивность (Реакция на изменения в рабочем процессе/правилах)", 
            "Вектор развития (Мотивация к освоению нового)", 
            "Deep Dive"
        ]
        curr_task = scenario[min(step-1, len(scenario)-1)]
        
        return f"""Ты — HR-коуч и эксперт по развитию персонала. Идет аудит сотрудника: {pos}.{context_block} Шаг: {step}/{max_steps}. Этап оценки: {curr_task}.{grade_adaptation}
        
        [ФОРМАТ ВОПРОСА]
        ЗАПРЕЩЕНО писать внутренние комментарии.
        Твой ответ должен выглядеть СТРОГО так:
        **Ситуация:** (1 короткая вводная мысль по этапу, строго в рамках специфики должности)
        **Вопрос:** (1 открытый вопрос, заставляющий рефлексировать о своем опыте)
        
        [БРОНЯ] Если сотрудник уходит от темы — МЯГКО верни его к рабочему контексту без оценки прошлых слов."""

def get_final_analysis_prompt(role, pos, jd_context, transcript):
    context_block = f"\n[БИЗНЕС-КОНТЕКСТ (ISO 30405)]:\n{jd_context}\n" if jd_context else ""
    anti_break_rule = "\n[КРИТИЧЕСКАЯ ИНСТРУКЦИЯ] ЗАПРЕЩЕНО использовать фигурные скобки { и } в тексте. Используй их ТОЛЬКО один раз в самом конце для JSON. ЗАПРЕЩЕНО рисовать текстовые таблицы и использовать Markdown-таблицы."
    
    if role == "Соискатель":
        return f"""Ты — объективный HR-директор. Проведи аудит интервью на позицию {pos}.{context_block}{anti_break_rule}
        
        [МЕТОДОЛОГИЯ ОЦЕНКИ BARS]
        1-3: Отвечает общими фразами, уходит от сути, некомпетентен.
        4-6: Знает базу, но плавает в нестандартных ситуациях.
        7-8: Приводит конкретные примеры, решает задачи алгоритмически верно.
        9-10: Предлагает оптимальные решения, мыслит проактивно (на уровень выше должности).
        ...
        return f"""Ты — корпоративный HR-методолог. Проведи аудит действующего сотрудника на позиции: {pos}.{context_block}{anti_break_rule}
        
        [МЕТОДОЛОГИЯ ОЦЕНКИ BARS]
        1-3: Выгорание, сопротивление изменениям.
        4-6: Исполнитель. Выполняет норму, но безынициативен.
        7-8: Самостоятелен, надежен (Готов к усложнению задач/переводу в наставники).
        9-10: Проактивен, берет ответственность за других (Лидер/HiPo).
        
        [СТРУКТУРА ОТВЕТА]
        ШАГ 1: ТЕКСТОВЫЙ ОТЧЕТ (Пиши строго сплошным текстом и списками, без таблиц!)
        - Матрица талантов (Результативность и Потенциал): Текстовое описание того, как соотносятся текущие результаты сотрудника и его потенциал к росту (например, "Стабильный профессионал", "Будущая звезда", "В зоне риска").
        - Дефицит компетенций (Skill-Gap): Чего не хватает для перехода на следующий логичный уровень (старший смены, эксперт и т.д.).
        - Индивидуальный план развития (ИПР): 3 конкретных, понятных шага на русском языке.
        - Вердикт.
